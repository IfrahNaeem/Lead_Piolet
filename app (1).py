        ...
        if gmail_client.is_connected():
            ...
            if gmail_client.has_client_config():
                ...
                if st.session_state.get("gmail_auth_url"):
                    ...
                    st.caption(
                        "After you log in and click Allow, Google will send your browser to a page "
                        "that fails to load — something like 'localhost refused to connect'. **That's "
                        "expected.** Just copy the full URL from your browser's address bar at that "
                        "point (it will contain `code=...`) and paste it below."
                    )
